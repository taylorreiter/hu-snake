...
import_kegg <- function(file, gsub_regex, origin){
  ...
  # gsub_regex: regex to use on locus_tag column to create bin identifier. For crumbs, "(_SRR1976948.[0-9_]{2,12})". For bins, "(_[0-9]*)"
...
# crumb assemblies that have marker genes for which there was no ortholog in the query
novel_marks <- anti_join(unique(crumb_marker[ , c('bin', 'name')]), unique(bin_marker[ , c('bin', 'name')]))
