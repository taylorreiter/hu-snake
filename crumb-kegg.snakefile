rule download_kegg_uni_crumbs:
    output:        
        ann = 'outputs/hu-crumbs-bin/unitigs/GhostKOALA/user_ko_definition.txt',
        tax = 'outputs/hu-crumbs-bin/unitigs/GhostKOALA/user.out.top'  
    shell:'''
...
rule split_kegg_uni_crumbs:
    output:
        ann = dynamic('outputs/hu-crumbs-bin/unitigs/GhostKOALA/{crumb_bin}.ko-ann-full.txt'),
        tax = dynamic('outputs/hu-crumbs-bin/unitigs/GhostKOALA/{crumb_bin}.ko-tax')  
    input: 
        ann = 'outputs/hu-crumbs-bin/unitigs/GhostKOALA/user_ko_definition.txt',
        tax = 'outputs/hu-crumbs-bin/unitigs/GhostKOALA/user.out.top'  
    run:
        ...
        for sample, hu_bin in hu.groupby(6):
            hu_bin.to_csv(f"outputs/hu-crumbs-bin/unitigs/GhostKOALA/{sample}.ko-ann-full.txt", index = False, header = False, sep = "\t")
        ...
        for sample, hu_bin in hu.groupby(7):
            hu_bin.to_csv(f"outputs/hu-crumbs-bin/unitigs/GhostKOALA/{sample}.ko-tax", index = False, header = False, sep = "\t")
...
rule plot_kegg_taxonomy_unitigs:
    output: 'outputs/hu-crumbs-bin/unitigs/GhostKOALA-plots/{crumb_bin}.pdf'
    input: 'outputs/hu-crumbs-bin/unitigs/GhostKOALA/{crumb_bin}.ko-tax'
    run:
        import pandas as pd
        import matplotlib
...
rule plot_kegg_annotations_unitigs:
    output: 
        plot1='outputs/hu-crumbs-bin/unitigs/GhostKOALA-plots/kegg-each.pdf',
        plot2a= 'outputs/hu-crumbs-bin/unitigs/GhostKOALA-plots/kegg-all-col-crumb-bin.pdf',
        plot2b= 'outputs/hu-crumbs-bin/unitigs/GhostKOALA-plots/kegg-all-col-kingdom.pdf'
    input:
        files = dynamic('outputs/hu-crumbs-bin/unitigs/GhostKOALA/{crumb_bin}.ko-ann-full.txt'),
        info = 'sandbox/hu_info.csv'
    params: dir = "outputs/hu-crumbs-bin/unitigs/GhostKOALA"
    conda: 'env-kegg.yml'
    shell:'''
...
rule plot_an_annotations_unitigs:
    output:
        tsv='outputs/hu-crumbs-bin/unitigs/an-et-al/an_uni.tsv',
        plot='outputs/hu-crumbs-bin/unitigs/an-et-al/an_uni.pdf'
    input:
        an ='sandbox/an-et-al/an_et_al_genes_parsed.tsv',
        files = dynamic('outputs/hu-crumbs-bin/unitigs/GhostKOALA/{crumb_bin}.ko-ann-full.txt')
    params: 
        title = "Unitigs",
        dir = 'outputs/hu-crumbs-bin/unitigs/GhostKOALA/'
    conda: 'env-skimr.yml'
    shell:'''
